...
export default function ActivityTimeline({ opportunityId }) {
  ...
  async function load() {
    setLoading(true);
    try {
      setItems((await api.listActivities(opportunityId)) || []);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  async function add() {
    ...
    setError('');
    try {
      await api.addActivity(opportunityId, { tipo, data, descrizione: descrizione.trim() });
      setDescrizione('');
      setData(todayISO());
      await load();
    } catch (e) {
  ...
  async function remove(id) {
    ...
    try {
      await api.deleteActivity(opportunityId, id);
  ...
  return (
    <div className="mt-6 border-t border-slate-100 pt-5">
      ...
      {/* Add form (not a <form> element — avoids nesting inside the modal's form) */}
      <div className="mb-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
        <div className="mb-2 grid grid-cols-2 gap-2">
          <select className={field} value={tipo} onChange={(e) => setTipo(e.target.value)}>
            {ACTIVITY_TIPI.map((t) => (
              <option key={t} value={t}>
                {ACTIVITY_TIPO_META[t].label}
              </option>
            ))}
          </select>
        ...
        <div className="flex gap-2">
          <input
            className={field}
            placeholder="Cosa è successo? (es. chiamato, fissato appuntamento…)"
            value={descrizione}
          ...
          <button
            ...
            onClick={add}
            disabled={busy}
            className="shrink-0 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:opacity-60"
          >
            {busy ? '…' : 'Aggiungi'}
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="py-4 text-center text-sm text-slate-400">
          Nessuna attività registrata. Logga la prima interazione qui sopra.
        </div>
      ) : (
            ...
            return (
              <li key={a.id} className="group flex items-start gap-3">
                <div className="mt-0.5 flex flex-col items-center">
                  <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-semibold uppercase ${meta.badge}`}>
                    {meta.label}
                  </span>
                </div>
